...
public class BruteForceSample : MonoBehaviour
{
    ...
    public void OnStartButtomClicked()
    {
        if (runnungRoutine != null)
        {
            Debug.Log("[Brute] 이미 실행중입니다.");
            return;
        }
        runnungRoutine = StartCoroutine(BruteForceRoutine());
    }
    ...
    IEnumerator BruteForceRoutine()
    {
        ...
        for (int i = 0; i < max; i++)
        {
            ...
            if (tryString == secretPin)
            {
                ...
                Debug.Log($"[Bruth] 성공 ! PIN={tryString} 시도수={tryCount} 소요={seconds:F3}초");
                runnungRoutine = null;
                yield break;
            }
            if (i % 100 == 0)
        ...
        sw.Stop();
        Debug.Log($"[Bruth] 모든 조합 시도 완료(발견 실패). 소요={sw.Elapsed.TotalMilliseconds:F3}초");
        runnungRoutine = null;
    }
}
